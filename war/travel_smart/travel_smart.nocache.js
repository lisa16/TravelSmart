...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "travel_smart" because\n';
  ...
  $wnd.__gwt_activeModules['travel_smart'] = {
    'moduleName' : 'travel_smart',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/travel_smart.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['travel_smart'].superdevmode = true;

  var expectedSuffix = '/travel_smart.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + travel_smart + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:travel_smart';
  var devModeSessionKey = '__gwtDevModeSession:travel_smart';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/travel_smart/travel_smart.nocache.js';
  ...
  $wnd.__gwt_activeModules['travel_smart'].canRedirect = true;
  $wnd.__gwt_activeModules['travel_smart'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING travel_smart';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/travel_smart?user.agent=' + ua + '&_callback=' + callback;
